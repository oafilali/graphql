...
function updateProjectStatistics(completedProjects) {
    ...
    const passed = completedProjects.filter(project => project.grade >= 1).length;
...
function drawXPGrowthChart(xpData) {
    ...
    const bottomGridLine = document.createElementNS("http://www.w3.org/2000/svg", "line");
    ...
    dataPoints.forEach(function(point, index) {
        ...
        const x = margin.left + ((point.date - minDate) / (maxDate - minDate)) * chartWidth;
    ...
    const startDateLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
    ...
    const endDateLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
    ...
    title.textContent = "XP Growth Over Time: " + totalXP.toLocaleString() + " XP";
...
function drawVerticalProjectsTimeline(completedProjects) {
    ...
    completedProjects.forEach(function(project, i) {
        ...
        if (i % 2 === 0) {
            ...
            label.textContent = projectName + " [" + formatDate(project.createdAt) + "]";
